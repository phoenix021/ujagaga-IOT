...
def thread_beacon():
    ...
    while not stop_flag:
        try:
            (msg, (sender_addr, sender_port)) = broadcaster.recvfrom(24)

            print("Received:", msg, " From:", sender_addr, "Port:", sender_port)

            if MSG_PING in msg.decode("utf-8"):
                broadcaster.sendto(MAC.encode(), (sender_addr, sender_port))
                print("Responding with: ", MAC)
        except:
            try:
                broadcaster.close()
            except:
            ...
            try:
                ...
                broadcaster.settimeout(20)
                broadcaster.bind(("", BROADCAST_PORT))
            except:
                pass
...
print("Running on IP: ", IP, ":", BROADCAST_PORT + 1)
